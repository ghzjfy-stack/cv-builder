(function (global) {
    ...
    function showImportToast(msg) {
        ...
        el.textContent = msg || t('importOk', 'הפרטים חולצו בהצלחה! אפשר לערוך אותם בטופס');
    ...
    function applyImport(parsed) {
        ...
        var nameField = document.getElementById('in-name');
        if (nameField && typeof nameField.scrollIntoView === 'function') {
            nameField.scrollIntoView({ block: 'nearest', behavior: 'smooth' });
        }
    ...
    function importBuffer(name, buffer, kind) {
        var job;
        if (kind === 'pdf') job = extractPdfText(buffer);
        else if (kind === 'docx') job = extractDocxText(buffer);
    ...
    function onFile(file) {
        ...
        if (!kind) {
            setStatus(t('importType', 'אפשר להעלות PDF או Word (.docx) בלבד.'), 'error');
            return Promise.resolve();
        }
        setStatus(t('importBusy', 'קוראים את הקובץ…'), 'busy');
        return file.arrayBuffer().then(function (buf) {
            return importBuffer(file.name, buf, kind);
        }).then(function () {
            setStatus('', '');
            showImportToast(t('importOk', 'הפרטים חולצו בהצלחה! אפשר לערוך אותם בטופס'));
        }).catch(function (err) {
            var code = err && err.message;
            if (code === 'type') setStatus(t('importType', 'אפשר להעלות PDF או Word (.docx) בלבד.'), 'error');
            else if (code === 'empty') setStatus(t('importScanned', 'לא נמצא טקסט בקובץ (ייתכן שהוא סרוק). נסו Word או הדביקו טקסט.'), 'error');
            else if (code === 'jszip' || code === 'pdfjs') setStatus(t('importFail', 'לא הצלחנו לקרוא את הקובץ. נסו PDF, או הדביקו טקסט.'), 'error');
    ...
    function bind() {
        ...
        if (button && !button.__qcImportBound) {
            button.__qcImportBound = true;
            button.addEventListener('click', function (e) {
                if (e && e.target && e.target.id === 'cv-import-file') return;
                pick();
            });
        }
