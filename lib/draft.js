(function (global) {
    ...
    function currentPreviewBg() {
        if (global.QCPreviewBg) return global.QCPreviewBg;
    ...
    function save() {
        if (global.QCDummy && QCDummy.active && !QCDummy.dirty) return;
        var data = {};
        ...
        data.accent = (typeof global.readCvAccentVar === 'function' && global.readCvAccentVar())
            || (function () {
                var wrap = document.getElementById('cv-preview-wrapper') || document.getElementById('cv-preview-stack');
                if (!wrap) return document.documentElement.style.getPropertyValue('--accent') || '';
                return wrap.style.getPropertyValue('--accent')
                    || wrap.style.getPropertyValue('--cv-accent-color')
                    || '';
            })();
        data.font = document.documentElement.style.getPropertyValue('--cv-font') || '';
        data.density = global.QCCvDensity || document.documentElement.getAttribute('data-cv-density') || 'regular';
    ...
    function apply(data) {
        ...
        var phrase = document.getElementById('phrase-field');
        if (phrase && data['phrase-field'] != null) phrase.value = data['phrase-field'];
    ...
    function isDraftField(el) {
        if (!el) return false;
        if (el.id && (FIELDS.indexOf(el.id) >= 0 || el.id === 'phrase-field' || el.id === 'font-scale' || el.id === 'line-height' || el.id === 'include-cover-letter')) {
            return true;
        }
        if (el.classList && el.classList.contains('studio-field')) return true;
        if (el.hasAttribute && (el.hasAttribute('data-job-field') || el.hasAttribute('data-entry-field') || el.hasAttribute('data-job-desc'))) {
            return true;
        }
        // Any form control inside the studio details / design panels.
        if (el.closest && el.closest('#details-panel, #design-panel, #studio-form-column, #experience-items, #education-items, #military-items')) {
    ...
    function bindLifecycle() {
        ...
        document.addEventListener('input', function (e) {
            if (isDraftField(e.target)) saveSoon();
        }, true);
        ...
        // Periodic safety net so refreshes mid-session never lose recent typing.
        if (!global.__qcDraftInterval) {
            global.__qcDraftInterval = setInterval(function () {
                try { save(); } catch (err) { /* ignore */ }
            }, 15000);
        }
    }
