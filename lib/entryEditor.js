    ...
    function createSectionEditor(config) {
        ...
        function render() {
            ...
            items.forEach(function (item, index) {
                ...
                remove.type = 'button';
                remove.className = 'exp-block-remove';
                ...
                remove.innerHTML = trashSvg() + '<span>' + (pack.remove || 'מחק') + '</span>';
                if (items.length <= 1) {
        ...
        function onAddEntryClick(e) {
            ...
            if (evt && evt.__qcEntryHandled) return false;
        ...
        function bindAddButton() {
            ...
            addBtn.removeAttribute('data-qc-add-bound');
            try { delete addBtn.dataset.qcAddBound; } catch (err) { /* ignore */ }
            ...
            addBtn.style.setProperty('z-index', '130', 'important');
            addBtn.classList.add('exp-add-bar');
            addBtn.classList.remove('section-add-btn');
            if (name === 'education') {
            ...
            addBtn.onclick = onAddEntryClick;
            addBtn.setAttribute('onclick', name === 'education'
                ? 'return window.__qcClickAddEducation ? window.__qcClickAddEducation(event) : (window.handleAddEducation ? window.handleAddEducation(event) : false)'
                : 'return false');
        }
        ...
        function bindDocumentAdd() {
            ...
            document.addEventListener('click', function (e) {
                ...
                var btn = t.closest('#' + config.addBtnId);
                if (!btn) return;
                onAddEntryClick(e);
            }, true);
        }
        ...
        function addItem(e) {
            ...
            try {
                ...
                if (lastCard) {
                    lastCard.classList.add('is-just-added');
                    window.setTimeout(function () {
                        try { lastCard.classList.remove('is-just-added'); } catch (flashErr) { /* ignore */ }
                    }, 1200);
                    try { lastCard.scrollIntoView({ behavior: 'smooth', block: 'center' }); } catch (scrollErr) { /* ignore */ }
                }
                var last = lastCard && lastCard.querySelector('input, textarea');
                if (last && typeof last.focus === 'function') {
                    try { last.focus({ preventScroll: true }); } catch (err) {
                        try { last.focus(); } catch (err2) { /* ignore */ }
                    }
                }
            } finally {
                addInFlight = false;
