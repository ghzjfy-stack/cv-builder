(function (global) {
    ...
    function createSectionEditor(config) {
        ...
        function publishEntries() {
            ...
            global.QCCvData[name] = items.map(toPreviewItem);
            if (name === 'military') {
        ...
        function handleEntryChange(idOrIndex, field, value) {
            ...
            if (index < 0) return;
            while (items.length <= index) items = items.concat([emptyItem()]);
        ...
        function refreshFromTextarea() {
            if (syncing) return;
            collectCards();
            var blanks = items.filter(isBlank);
            var el = fieldEl();
            var parsed = parseItems(el ? el.value : '');
            var filled = parsed.filter(function (row) { return !isBlank(row); });
            if (filled.length) items = filled.concat(blanks);
            else items = blanks.length ? blanks : [emptyItem()];
            ensureItems();
            if (!rawMode) render();
        }
        ...
        function addItem(e) {
            ...
            var now = Date.now();
            if (now - addLock < 250) return false;
            addLock = now;
            collectCards();
            ensureItems();
            items = items.concat([{
