(function (global) {
    ...
    function createSectionEditor(config) {
        ...
        var MAX_SLOTS = config.maxSlots || (config.hideAdd ? 1 : 40);
        var DEFAULT_VISIBLE = config.defaultVisible || (config.hideAdd ? 1 : 2);
        ...
        function neededSlots() {
            if (visibleCount < 1) visibleCount = DEFAULT_VISIBLE;
            if (visibleCount > MAX_SLOTS) visibleCount = MAX_SLOTS;
            return visibleCount;
        }
        ...
        function ensureItems() {
            ...
            needed = neededSlots();
            for (i = 0; i < needed; i++) {
                if (!items[i]) items[i] = emptyItemAt(i);
                else {
                    items[i] = normalize(items[i]);
                    items[i].id = slotId(i);
                }
            }
            if (items.length > needed) items = items.slice(0, needed);
            return items;
        }
        ...
        function toPreviewItem(item) {
            var company = String((item && (item.company || item.org || item.employer)) || '');
            var position = String((item && (item.position || item.title || item.role)) || '');
        ...
        function publishEntries() {
            ...
            global.QCCvData[name] = items.slice(0, visibleCount).map(toPreviewItem).filter(function (row) {
        ...
        function setEntryField(item, field, value) {
            ...
            if (field === 'company' || field === 'org' || field === 'employer') {
                item.company = val;
                item.org = val;
                item.employer = val;
                return;
            }
            if (field === 'position' || field === 'title' || field === 'role') {
                item.position = val;
                item.title = val;
                item.role = val;
                return;
        ...
        function render() {
            ...
            items.slice(0, visibleCount).forEach(function (item, index) {
                ...
                notes.value = item.notes || item.description || '';
                ...
                var orgField = labeled(pack.org, makeInput('data-entry-org', pack.orgPh, item.org || item.company || '', 'company'));
                var titleField = labeled(pack.title, makeInput('data-entry-title', pack.titlePh, item.title || item.position || '', 'position'));
                var datesField = labeled(pack.dates, makeInput('data-entry-dates', pack.datesPh, item.dates || item.years || '', 'years'));
        ...
        function refreshFromTextarea() {
            ...
            items = [];
            if (config.hideAdd) visibleCount = 1;
            else visibleCount = Math.min(MAX_SLOTS, Math.max(DEFAULT_VISIBLE, filled.length || DEFAULT_VISIBLE));
            ensureItems();
            var i;
            for (i = 0; i < filled.length && i < MAX_SLOTS; i++) {
                items[i] = normalize(filled[i]);
                items[i].id = slotId(i);
            }
            if (!rawMode) render();
        }
        ...
        function applyRawUi() {
            ...
            if (addBtn) {
                if (config.hideAdd) hideAddButton();
                    ...
                    addBtn.hidden = visibleCount >= MAX_SLOTS;
                    if (visibleCount >= MAX_SLOTS) addBtn.setAttribute('hidden', '');
                    else {
        ...
        function bindAddButton() {
            ...
            addBtn.disabled = visibleCount >= MAX_SLOTS;
            addBtn.hidden = visibleCount >= MAX_SLOTS;
            if (visibleCount >= MAX_SLOTS) addBtn.setAttribute('hidden', '');
            else {
                addBtn.removeAttribute('hidden');
            ...
            addBtn.style.setProperty('border-color', '#0284c7', 'important');
        ...
        function addItem(e) {
            ...
            if (e && typeof e.stopPropagation === 'function') e.stopPropagation();
            collectCards();
            visibleCount = Math.min((visibleCount || DEFAULT_VISIBLE) + 1, MAX_SLOTS);
            ensureItems();
            render();
            writeField(true);
        ...
        function removeItem(id) {
            ...
            if (!Number.isFinite(index) || index < 0 || index >= visibleCount) return;
            var i;
            for (i = index; i < visibleCount - 1; i++) {
                items[i] = normalize(items[i + 1] || emptyItemAt(i));
                items[i].id = slotId(i);
            }
            items[visibleCount - 1] = emptyItemAt(visibleCount - 1);
        ...
        function bind() {
            ...
            ensureItems();
            var fi;
            for (fi = 0; fi < filled.length && fi < MAX_SLOTS; fi++) {
                items[fi] = normalize(filled[fi]);
                items[fi].id = slotId(fi);
            }
            render();
            applyI18n();
            ensureAddButtonVisible();
        ...
        var api = {
            ...
            updateMilitary: function (field, value) {
                return handleEntryChange(0, field, value);
            },
    ...
    global.updateMilitary = function (nameOrEvent, value) {
        ...
        if (nameOrEvent && nameOrEvent.target) {
            field = nameOrEvent.target.name || nameOrEvent.target.getAttribute('data-entry-field');
