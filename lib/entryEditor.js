(function (global) {
    ...
    function createSectionEditor(config) {
        ...
        function emptyItem() {
            ...
            if (name === 'education' && (!items || !items.length)) id = '1';
            return {
                id: id,
                dates: '',
                years: '',
        ...
        function ensureItems() {
            ...
            items = items.map(function (row) { return normalize(row); });
        ...
        function handleEntryChange(idOrIndex, field, value) {
            ...
            if (config.layout === 'military') {
                ...
                var row = items[index];
                var roleEl = host && host.querySelector('[name="role"], [data-entry-field="position"], [data-entry-title]');
                var yearsEl = host && host.querySelector('[name="dates"], [data-entry-field="years"], [data-entry-dates]');
                if (roleEl) roleEl.value = (row && (row.role || row.title || row.position)) || '';
                if (yearsEl) yearsEl.value = (row && (row.years || row.dates)) || '';
            }
            writeField(true);
        }
        ...
        function render() {
            ...
            items.forEach(function (item, index) {
                var id = item.id || Date.now().toString();
                item.id = id;
        ...
        function refreshFromTextarea(opts) {
            if (syncing) return;
            if (config.layout !== 'military') collectCards();
            if (opts && opts.replaceFromRaw) {
                ...
                if (config.hideAdd) {
                    ...
                    items = filled.length ? filled.map(normalize) : [emptyItem()];
                }
            }
            ensureItems();
        ...
        function bind() {
            ...
            visibleCount = items.length;
            ensureItems();
            if (config.layout !== 'military') render();
            applyI18n();
            ensureAddButtonVisible();
            ...
            function wrap(fnName) {
                ...
                var wrapped = function () {
                    ...
                    rawMode = false;
                    refreshFromTextarea({
                        replaceFromRaw: fnName === 'fillCvSample' || fnName === 'loadExample' || fnName === 'loadRoleExample' || fnName === 'replaceRoleExample' || fnName === 'clearCvForm' || fnName === 'resetForm'
                    });
                    applyI18n();
                    return result;
                };
        ...
        var api = {
            ...
            updateMilitary: function (field, value) {
                ensureItems();
                var mapped = field;
                if (field === 'years' || field === 'dates' || field === 'date') mapped = 'years';
                else if (field === 'role' || field === 'title' || field === 'position') mapped = 'role';
                return handleEntryChange((items[0] && items[0].id) || '1', mapped, value);
            },
    ...
    if (!Array.isArray(global.QCCvData.education) || global.QCCvData.education.length < 1) {
        global.QCCvData.education = [{ id: '1', institution: '', degree: '', dates: '' }];
    }
