(function (global) {
    ...
    function createSectionEditor(config) {
        ...
        function parseItems(text) {
            ...
            if (!raw.replace(/[\u200b\u200c\u200d\s|]/g, '')) return [emptyItem()];
        ...
        function render() {
            ...
            items.forEach(function (item, index) {
                ...
                if (config.orgFirst === false) fields.append(titleField, orgField, datesField);
        ...
        function addItem(e) {
            ...
            try {
                ...
                if (last && last.closest && last.closest('.entry-card') && typeof last.closest('.entry-card').scrollIntoView === 'function') {
                    last.closest('.entry-card').scrollIntoView({ behavior: 'smooth', block: 'nearest' });
                }
            } finally {
                adding = false;
            }
    ...
    if (!document.documentElement.dataset.qcSectionAdd) {
        ...
        document.addEventListener('click', function (e) {
            var btn = e.target && e.target.closest && e.target.closest('#btn-add-education, #btn-add-military, #btn-add-experience');
            if (!btn) return;
