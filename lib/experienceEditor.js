(function (global) {
    ...
    function handleExperienceChange(idOrIndex, field, value) {
        ...
        if (index < 0) return;
        while (jobs.length <= index) jobs.push(emptyJob());
        if (!jobs[index]) jobs[index] = emptyJob();
        setJobField(jobs[index], field, value);
    ...
    function toPreviewJob(job) {
        ...
        var dates = String((job && (job.dates || job.date)) || '').trim();
    ...
    function renderJobs() {
        ...
        jobs.forEach(function (job, jobIndex) {
            ...
            var datesInput = makeInput('data-job-dates', pack.datesPh, job.dates, 'dates');
    ...
    function refreshJobsFromTextarea() {
        if (syncing) return;
        collectCardsIntoJobs();
        var blanks = jobs.filter(isBlankJob);
        var el = experienceEl();
        var parsed = parseJobs(el ? el.value : '');
        var filled = parsed.filter(function (job) { return !isBlankJob(job); });
        if (filled.length) jobs = filled.concat(blanks);
        else jobs = blanks.length ? blanks : [emptyJob()];
        ensureJobs();
    ...
    function addExperience(e) {
        ...
        var now = Date.now();
        if (now - addJobLock < 250) return false;
        addJobLock = now;
        collectCardsIntoJobs();
        ensureJobs();
        jobs = jobs.concat([normalizeJob({
            id: now,
            company: '',
            position: '',
            years: '',
    ...
    function patchCardI18n() {
        ...
        Array.prototype.forEach.call(cards, function (card, jobIndex) {
            ...
            var labels = card.querySelectorAll('.experience-field-label');
            if (labels[0]) labels[0].textContent = pack.employer;
            if (labels[1]) labels[1].textContent = pack.role;
            if (labels[2]) labels[2].textContent = pack.dates;
            if (labels[3]) labels[3].textContent = pack.description;
    ...
    function bindAddButton() {
        ...
        addJobBtn.addEventListener('click', addExperience);
    }
    ...
    if (!document.documentElement.dataset.qcExpAdd) {
        document.documentElement.dataset.qcExpAdd = '1';
        document.addEventListener('click', function (e) {
            var btn = e.target && e.target.closest && e.target.closest('#btn-add-experience');
            if (!btn) return;
            addExperience(e);
        }, true);
    }
