(function (global) {
    ...
    function renderJobs() {
        ...
        jobs.forEach(function (job, jobIndex) {
            ...
            removeBtn.textContent = pack.removeJob || 'הסר';
    ...
    function addExperience(e) {
        ...
        if (evt) {
            ...
            // Do NOT stopImmediatePropagation — it killed sibling handlers and left
            // buttons looking dead when the first path no-op'd.
        }
        addInFlight = true;
        try {
    ...
    function patchCardI18n() {
        ...
        Array.prototype.forEach.call(cards, function (card, jobIndex) {
            ...
            if (remove) {
                ...
                if (remove.classList.contains('exp-block-remove')) remove.textContent = pack.removeJob;
            }
    ...
    function wireAddButtonEl(addJobBtn) {
        if (!addJobBtn) return;
        // Keep type=submit when inside the add form — form submit is the reliable path.
        var inForm = addJobBtn.closest && addJobBtn.closest('#experience-add-form');
        if (inForm) {
            addJobBtn.type = 'submit';
            addJobBtn.setAttribute('type', 'submit');
        } else {
        ...
        addJobBtn.style.setProperty('z-index', '120', 'important');
    }
    ...
    function bindAddButton() {
        ...
        // Hide legacy top button if an old cached HTML still has it.
        var top = document.getElementById('btn-add-experience-top');
        if (top) {
            top.hidden = true;
            top.style.display = 'none';
        }
        if (form && !form.__qcAddBound) {
            form.__qcAddBound = true;
    ...
    function bindDocumentAdd() {
        if (global.__qcAddJobDocBound === 'exp-ui1') return;
        global.__qcAddJobDocBound = 'exp-ui1';
        ...
        document.addEventListener('click', function (e) {
            ...
            if (!btn) return;
            // Let the form submit handler own the primary path when possible.
