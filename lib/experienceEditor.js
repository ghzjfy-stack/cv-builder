(function (global) {
    ...
    function renderJobs() {
        ...
        jobs.forEach(function (job, jobIndex) {
            ...
            card.append(
                toolbar,
                labeledField(pack.role, roleInput, 'job-role-' + jobIndex),
                labeledField(pack.employer, employerInput, 'job-employer-' + jobIndex),
                labeledField(pack.dates, datesRow, 'job-dates-' + jobIndex),
                labeledField(pack.description, desc, 'job-desc-' + jobIndex),
                presetsWrap
            );
    ...
    function refreshJobsFromTextarea() {
        ...
        var hasContent = parsed.some(function (job) { return !isBlankJob(job); });
        jobs = hasContent ? parsed.filter(function (job) { return !isBlankJob(job); }) : parsed;
        if (!jobs.length) jobs = [emptyJob()];
        renderJobs();
    }
    ...
    function addJob(e) {
        ...
        var now = Date.now();
        if (now - lastAddAt < 250) return false;
        lastAddAt = now;
        collectCardsIntoJobs();
        var newEntry = emptyJob();
        jobs.push(newEntry);
        console.log('Add Experience Clicked!', newEntry);
        renderJobs();
        writeExperience(jobs, true);
    ...
    function applyPreset(index, presetId) {
        ...
        var i;
        var now = Date.now();
        var key = String(index) + ':' + String(presetId);
    ...
    function bindAddButton() {
        ...
        if (addJobBtn) {
            ...
            addJobBtn.style.pointerEvents = 'auto';
            addJobBtn.onclick = addJob;
            if (!addJobBtn.__qcAddBound) {
                addJobBtn.__qcAddBound = true;
                addJobBtn.addEventListener('click', addJob);
            }
        }
        var section = document.getElementById('experience-section');
        if (section && !section.__qcAddBound) {
            section.__qcAddBound = true;
        ...
        if (!document.__qcAddExpBound) {
            document.__qcAddExpBound = true;
