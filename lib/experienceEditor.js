(function (global) {
    ...
    function addExperience(e) {
        ...
        // Guard against capture+target both firing on the same gesture.
        var now = Date.now();
        if (now - (addExperience._lastAt || 0) < 350) return false;
        addExperience._lastAt = now;
        addInFlight = true;
        try {
            ...
            ensureJobs();
            renderJobs();
            // Collapse older cards so the new one is obvious.
            var host = document.getElementById('experience-items');
            if (host) {
                Array.prototype.forEach.call(host.querySelectorAll('.experience-card'), function (card, i) {
                    var open = i === jobs.length - 1;
    ...
    function onAddExperienceClick(e) {
        var evt = e || window.event;
        if (evt && evt.__qcJobHandled) return false;
        if (evt && typeof evt.preventDefault === 'function') evt.preventDefault();
        return addExperience(evt);
    }
    ...
    function wireAddButtonEl(addJobBtn) {
        ...
        addJobBtn.style.setProperty('z-index', '90', 'important');
        addJobBtn.onclick = onAddExperienceClick;
        addJobBtn.setAttribute('onclick', 'return window.addExperience ? window.addExperience(event) : false');
    }

    function bindAddButton() {
        var addJobBtn = document.getElementById('btn-add-experience');
        var addTop = document.getElementById('btn-add-experience-top');
        if (addJobBtn) {
            addJobBtn.style.setProperty('display', 'inline-flex', 'important');
            addJobBtn.style.setProperty('position', 'relative', 'important');
        }
        wireAddButtonEl(addJobBtn);
        wireAddButtonEl(addTop);
    }

    function bindDocumentAdd() {
        if (global.__qcAddJobDelegated) return;
        global.__qcAddJobDelegated = true;
        document.addEventListener('click', function (e) {
            var t = e && e.target;
            if (!t || typeof t.closest !== 'function') return;
            var btn = t.closest('#btn-add-experience, #btn-add-experience-top');
            if (!btn) return;
            onAddExperienceClick(e);
        }, true);
    }
