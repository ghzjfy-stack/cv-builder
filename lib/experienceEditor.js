(function (global) {
    ...
    function addJob(e) {
        ...
        var now = Date.now();
        if (adding || now - lastAddAt < 400) return false;
        adding = true;
        lastAddAt = now;
        try {
    ...
    function clickHitsAddButton(e) {
        var btn = document.getElementById('btn-add-experience');
        if (!btn || !e) return false;
        var node = e.target;
        if (node && node.closest && node.closest('#btn-add-experience')) return true;
    ...
    function bindAddButton() {
        ...
        if (section && !section.__qcAddBound) {
            ...
            section.addEventListener('click', function (e) {
                if (!clickHitsAddButton(e)) return;
                addJob(e);
            }, true);
        }
    }
