(function (global) {
    ...
    function formatHeader(job) {
        ...
        var role = sanitize(job.role || '');
        var employer = sanitize(job.employer || '');
        if (dates || role || employer) {
            return [dates, role, employer].join(' | ');
        }
        return '';
    }
    ...
    function renderJobs() {
        ...
        jobs.forEach(function (job, jobIndex) {
            ...
            function makeInput(attr, placeholder, value) {
                ...
                input.type = 'text';
                input.className = 'w-full bg-slate-900 border border-slate-700 p-2.5 text-sm rounded-lg text-slate-100 outline-none min-h-11';
