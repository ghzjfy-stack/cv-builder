(function (global) {
    ...
    function refreshJobsFromTextarea(opts) {
        ...
        if (opts && opts.replaceFromRaw) {
            ...
            jobs = filled.length ? filled.map(normalizeJob) : [emptyJob('1')];
        }
        ensureJobs();
    ...
    function addExperience(e) {
        if (e && e.__qcJobAdded) return false;
        if (addInFlight) return false;
        var now = Date.now();
        if (lastAddAt && now - lastAddAt < 80) return false;
        addInFlight = true;
        try {
            if (e) {
    ...
    function bindAddButton() {
        ...
        addJobBtn.style.setProperty('z-index', '40', 'important');
    ...
    function bindDocumentAdd() {
        ...
        document.addEventListener('click', function (e) {
            ...
            if (!t.closest('#btn-add-experience')) return;
            if (e && typeof e.preventDefault === 'function') e.preventDefault();
            addExperience(e);
        }, true);
    }
    ...
    var api = {
        ...
        previewJobs: previewJobs,
            ...
            if (!host || cards < jobs.length) renderJobs();
            else {
