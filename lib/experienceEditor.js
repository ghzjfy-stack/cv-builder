(function (global) {
    ...
    function publishExperience() {
        ...
        global.QCCvData.experience = jobs.map(toPreviewJob).filter(function (job) {
            return job.company || job.position || job.dates || job.description || (job.bullets && job.bullets.length);
        });
    }
    ...
    function addExperience(e) {
        ...
        try {
            ...
            // Immutable append — never mutate/push the jobs array in place.
            var newId = Date.now().toString() + '-' + Math.random().toString(36).slice(2, 7);
            ...
            ensureJobs();
            renderJobs();
            // Persist filled jobs only in the hidden textarea, but keep empty cards
            // in memory/DOM so "Add another job" always stays visible.
            writeExperience(jobs, true);
            ensureAddButtonVisible();
    ...
    if (!Array.isArray(global.QCCvData.experience) || global.QCCvData.experience.length < 1) {
        global.QCCvData.experience = [{ id: 'job-seed-0', company: '', position: '', dates: '', description: '' }];
    }
