(function (global) {
    ...
    function ensureJobs() {
        ...
        jobs = jobs.map(function (job) { return normalizeJob(job); });
    ...
    function addExperience(e) {
        ...
        var now = Date.now();
        if (now - lastAddAt < 120) return false;
        lastAddAt = now;
        addInFlight = true;
        try {
            if (e) {
            ...
            while (jobIndexById(nextId) >= 0) nextId = Date.now().toString() + '-' + jobs.length;
            jobs = jobs.concat([emptyJob(nextId)]);
            visibleJobsCount = jobs.length;
            ensureJobs();
            renderJobs();
            writeExperience(jobs, true);
