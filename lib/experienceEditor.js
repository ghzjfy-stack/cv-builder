(function (global) {
    ...
    var jobs = [];
    var visibleJobsCount = 2;
    var MAX_JOBS = 40;
    var DEFAULT_VISIBLE_JOBS = 2;
    var syncing = false;
    var bound = false;
    var rawMode = false;
    ...
    function slotJobId(index) {
        return 'job-slot-' + index;
    }

    function emptyJobAt(index) {
        return normalizeJob({
            id: slotJobId(index),
            dates: '',
            title: '',
    ...
    function neededJobSlots() {
        if (visibleJobsCount < 1) visibleJobsCount = DEFAULT_VISIBLE_JOBS;
        if (visibleJobsCount > MAX_JOBS) visibleJobsCount = MAX_JOBS;
        return visibleJobsCount;
    }
    ...
    function ensureJobs() {
        ...
        needed = neededJobSlots();
        for (i = 0; i < needed; i++) {
            if (!jobs[i]) jobs[i] = emptyJobAt(i);
            else {
                jobs[i] = normalizeJob(jobs[i]);
                jobs[i].id = slotJobId(i);
            }
        }
        if (jobs.length > needed) jobs = jobs.slice(0, needed);
        return jobs;
    }
    ...
    function publishExperience() {
        ...
        global.QCCvData.experience = jobs.slice(0, visibleJobsCount).map(toPreviewJob).filter(function (job) {
    ...
    function renderJobs() {
        ...
        jobs.slice(0, visibleJobsCount).forEach(function (job, jobIndex) {
            var jobId = slotJobId(jobIndex);
            job.id = jobId;
    ...
    function refreshJobsFromTextarea() {
        ...
        var i;
        if (rawMode || filled.length) {
            visibleJobsCount = Math.min(MAX_JOBS, Math.max(DEFAULT_VISIBLE_JOBS, filled.length || DEFAULT_VISIBLE_JOBS));
            jobs = [];
            for (i = 0; i < visibleJobsCount; i++) {
                jobs[i] = filled[i] ? normalizeJob(filled[i]) : emptyJobAt(i);
                jobs[i].id = slotJobId(i);
            }
        }
        ensureJobs();
    ...
    function addExperience(e) {
        ...
        collectCardsIntoJobs();
        visibleJobsCount = Math.min((visibleJobsCount || DEFAULT_VISIBLE_JOBS) + 1, MAX_JOBS);
        ensureJobs();
        renderJobs();
        writeExperience(jobs, true);
        ensureAddButtonVisible();
        focusJobCard(visibleJobsCount - 1);
        return false;
    }
    ...
    function removeJob(idOrIndex) {
        collectCardsIntoJobs();
        var index = resolveJobIndex(idOrIndex);
        if (index < 0 || index >= visibleJobsCount) return;
        var i;
        for (i = index; i < visibleJobsCount - 1; i++) {
            jobs[i] = normalizeJob(jobs[i + 1] || emptyJobAt(i));
            jobs[i].id = slotJobId(i);
        }
        jobs[visibleJobsCount - 1] = emptyJobAt(visibleJobsCount - 1);
        if (visibleJobsCount > 1) visibleJobsCount -= 1;
        ensureJobs();
        renderJobs();
        writeExperience(jobs, true);
        ensureAddButtonVisible();
    ...
    function applyRawUi() {
        ...
        if (addBtn) {
            addBtn.hidden = visibleJobsCount >= MAX_JOBS;
            if (visibleJobsCount >= MAX_JOBS) addBtn.setAttribute('hidden', '');
            else addBtn.removeAttribute('hidden');
        }
    ...
    function ensureAddButtonVisible() {
        ...
        addJobBtn.hidden = visibleJobsCount >= MAX_JOBS;
        if (visibleJobsCount >= MAX_JOBS) addJobBtn.setAttribute('hidden', '');
        else addJobBtn.removeAttribute('hidden');
        bindAddButton();
    }
    ...
    function bindAddButton() {
        ...
        addJobBtn.disabled = visibleJobsCount >= MAX_JOBS;
        addJobBtn.hidden = visibleJobsCount >= MAX_JOBS;
        if (visibleJobsCount >= MAX_JOBS) addJobBtn.setAttribute('hidden', '');
        else {
    ...
    function bind() {
        ...
        visibleJobsCount = Math.min(MAX_JOBS, Math.max(DEFAULT_VISIBLE_JOBS, filled.length || DEFAULT_VISIBLE_JOBS));
        ensureJobs();
        for (var fi = 0; fi < filled.length && fi < MAX_JOBS; fi++) {
            jobs[fi] = normalizeJob(filled[fi]);
            jobs[fi].id = slotJobId(fi);
        }
        renderJobs();
        applyI18n();
        bindRawToggle();
    ...
    jobs = [];
    visibleJobsCount = DEFAULT_VISIBLE_JOBS;
    ensureJobs();
    function start() {
        try {
            bind();
