(function (global) {
    ...
    function wireAddButtonEl(addJobBtn) {
        ...
        // Prefer direct listener; keep a minimal inline fallback for cache mismatches.
        addJobBtn.onclick = null;
        addJobBtn.removeAttribute('onclick');
        if (!addJobBtn.__qcAddWired) {
            addJobBtn.__qcAddWired = true;
            addJobBtn.addEventListener('click', function (e) {
                addExperience(e || window.event);
            });
        }
    }
    ...
    function bindDocumentAdd() {
        // Versioned flag so a warm cache never leaves a dead delegated listener.
        if (global.__qcAddJobDocBound === 'add13') return;
        global.__qcAddJobDocBound = 'add13';
        global.__qcAddJobDelegated = true;
        document.addEventListener('click', function (e) {
            var t = e && e.target;
            if (!t || typeof t.closest !== 'function') return;
            var btn = t.closest('[data-qc-add-job], #btn-add-experience, #btn-add-experience-top');
            if (!btn) return;
            addExperience(e);
        }, true);
    }
