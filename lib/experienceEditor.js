(function (global) {
    ...
    function renderJobs() {
        ...
        jobs.forEach(function (job, jobIndex) {
            ...
            toggle.setAttribute('data-toggle-job', jobId);
    ...
    function applyI18n() {
        ...
        var addTopLabel = document.getElementById('btn-add-experience-top-label');
        if (addTopLabel) addTopLabel.textContent = pack.addJobShort || pack.addJob;
        applyRawUi();
        if (rawMode) return;
        var host = document.getElementById('experience-items');
    ...
    function wireAddButtonEl(addJobBtn) {
        ...
        addJobBtn.style.setProperty('display', 'inline-flex', 'important');
        addJobBtn.style.setProperty('pointer-events', 'auto', 'important');
        addJobBtn.style.setProperty('cursor', 'pointer', 'important');
        addJobBtn.style.setProperty('position', 'relative', 'important');
        addJobBtn.style.setProperty('z-index', '120', 'important');
        // NEVER strip inline onclick — that left buttons dead when listeners failed.
        addJobBtn.setAttribute(
            'onclick',
            'return window.__qcClickAddJob ? window.__qcClickAddJob(event) : false'
        );
        addJobBtn.onclick = function (e) {
            return global.__qcClickAddJob ? global.__qcClickAddJob(e || window.event) : false;
        };
    }

    function bindAddButton() {
        wireAddButtonEl(document.getElementById('btn-add-experience'));
        wireAddButtonEl(document.getElementById('btn-add-experience-top'));
    }

    function bindDocumentAdd() {
        if (global.__qcAddJobDocBound === 'add14') return;
        global.__qcAddJobDocBound = 'add14';
        ...
        document.addEventListener('click', function (e) {
            ...
            var btn = el.closest('[data-qc-add-job], #btn-add-experience, #btn-add-experience-top');
            if (!btn) return;
