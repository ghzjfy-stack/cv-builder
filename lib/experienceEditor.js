(function (global) {
    ...
    function focusJobCard(index) {
        ...
        if (formCol && typeof card.scrollIntoView === 'function') {
            try { card.scrollIntoView({ behavior: 'smooth', block: 'center' }); } catch (err) { /* ignore */ }
        } else if (typeof card.scrollIntoView === 'function') {
            try { card.scrollIntoView({ behavior: 'smooth', block: 'nearest' }); } catch (err) { /* ignore */ }
        }
    }
    ...
    function bindAddButton() {
        ...
        // Always reassign onclick so a replaced/cloned button stays live.
        // Document capture is the primary path; __qcJobHandled prevents double-add.
    ...
    function bindDocumentAdd() {
        ...
        // Capture-phase: survives button node replacement and overlay quirks.
    ...
    function bind() {
        ...
        visibleJobsCount = jobs.length;
        ensureJobs();
        if (!filled.length && document.querySelector('#experience-items .experience-card') && jobs.length <= 1) {
            collectCardsIntoJobs();
            ensureJobs();
            if (jobs[0]) jobs[0].id = jobs[0].id || 'job-seed-0';
            var seed = document.querySelector('#experience-items .experience-card');
            if (seed && jobs[0]) {
                seed.setAttribute('data-job-id', jobs[0].id);
