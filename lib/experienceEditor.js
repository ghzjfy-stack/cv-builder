(function (global) {
    ...
    var jobs = [];
    var visibleJobsCount = 1;
    var MAX_JOBS = 100;
    var DEFAULT_VISIBLE_JOBS = 1;
    var syncing = false;
    var bound = false;
    var rawMode = false;
    ...
    function ensureJobs() {
        if (!Array.isArray(jobs) || jobs.length < 1) jobs = [emptyJob('job-seed-0')];
        jobs = jobs.filter(Boolean).map(function (job) { return normalizeJob(job); });
    ...
    function toFormJob(job) {
        var n = normalizeJob(job || {});
        var role = n.role || n.title || n.position || '';
        var company = n.company || n.employer || '';
        var dates = n.dates || n.years || '';
        ...
        return {
            ...
            dates: dates,
            description: description,
            // Preview / legacy aliases
            position: role,
            employer: company,
            years: dates,
            bullets: Array.isArray(n.bullets) ? n.bullets.slice() : []
        };
    }

    function publishExperience() {
        global.QCCvData = global.QCCvData || {};
        // Main form state: keep every slot (including empty) so add/delete stays in sync.
        global.QCCvData.experience = (jobs || []).map(toFormJob);
        if (typeof global.__qcNotifyFormData === 'function') {
            try { global.__qcNotifyFormData(global.QCCvData); } catch (err) { /* React bridge optional */ }
        }
    }
    ...
    function writeExperience(list, triggerUpdate) {
        ...
        if (el) {
            ...
            // Persist trailing empty slots outside the CV text so draft/raw
            // reloads keep cards the user added but has not filled yet.
            el.setAttribute('data-qc-blank-jobs', String(trailingBlankJobs().length));
            syncing = false;
        }
    ...
    function readJobFromCard(card, jobIndex) {
        ...
        var dates = card.querySelector('[data-job-dates], [data-job-field="years"]');
        var role = card.querySelector('[data-job-role], [data-job-field="position"]');
        var employer = card.querySelector('[data-job-employer], [data-job-field="company"]');
        var desc = card.querySelector('[data-job-desc], [data-job-field="description"]');
        if (!dates && !role && !employer && !desc) return;
        jobs[jobIndex].dates = dates ? dates.value : '';
        jobs[jobIndex].years = jobs[jobIndex].dates;
    ...
    function renderJobs() {
        ...
        jobs.forEach(function (job, jobIndex) {
            // Unique key: prefer stable id, fall back to index (never collide with Date.now).
            var jobId = String((job && job.id) || ('job-' + jobIndex));
            if (job && !job.id) job.id = jobId;
            var company = (job && (job.company || job.employer)) || '';
            var role = (job && (job.role || job.position || job.title)) || '';
            var years = (job && (job.years || job.dates)) || '';
            var description = (job && job.description) || descriptionFromBullets(job && job.bullets) || '';
            var card = document.createElement('article');
            card.className = 'experience-card exp-block is-open';
            card.setAttribute('role', 'listitem');
            card.setAttribute('data-job-id', jobId);
            card.setAttribute('data-job-index', String(jobIndex));
            card.setAttribute('data-job-key', jobId);

            var toolbar = document.createElement('div');
            toolbar.className = 'exp-block-head';
            var badge = document.createElement('span');
            badge.className = 'experience-card-badge exp-block-num';
            badge.textContent = pack.jobN + ' ' + (jobIndex + 1);
            var actions = document.createElement('div');
            actions.className = 'experience-card-actions';
            var removeBtn = document.createElement('button');
            removeBtn.type = 'button';
            removeBtn.className = 'exp-block-remove';
            removeBtn.setAttribute('data-remove-job', jobId);
            removeBtn.innerHTML = trashSvg() + '<span>' + (pack.removeJob || 'הסר') + '</span>';
            removeBtn.setAttribute('aria-label', pack.removeJob || 'הסר');
            removeBtn.title = pack.removeJob || 'הסר';
            if (jobs.length <= 1) {
            ...
            var roleInput = makeInput('data-job-role', pack.rolePh, role, 'position');
            var employerInput = makeInput('data-job-employer', pack.employerPh, company, 'company');

            var datesRow = document.createElement('div');
            datesRow.className = 'experience-dates-row';
            var datesInput = makeInput('data-job-dates', pack.datesPh, years, 'years');
            ...
            fields.className = 'experience-card-fields exp-block-fields';
            fields.append(
        ...
        Array.prototype.forEach.call(host.querySelectorAll('.experience-card'), function (card, i) {
            syncPresetChipState(card, jobs[i] && jobs[i].description);
        });
    ...
    function collectCardsIntoJobs() {
        ...
        Array.prototype.forEach.call(cards, function (card) {
            ...
            var id = card.getAttribute('data-job-id') || nextJobId();
            next.push(emptyJob(id));
        });
        jobs = next.length ? next : [emptyJob('job-seed-0')];
        Array.prototype.forEach.call(host.querySelectorAll('.experience-card'), function (card, i) {
            if (!jobs[i]) return;
            if (!card.querySelector('[data-job-employer], [data-job-role], [data-job-dates], [data-job-desc]')) return;
            var id = card.getAttribute('data-job-id') || jobs[i].id;
            jobs[i].id = id;
            card.setAttribute('data-job-id', id);
            card.setAttribute('data-job-index', String(i));
            readJobFromCard(card, i);
        });
        if (jobs.length > MAX_JOBS) jobs = jobs.slice(0, MAX_JOBS);
        visibleJobsCount = jobs.length;
    }
    ...
    function refreshJobsFromTextarea(opts) {
        if (syncing) return;
        collectCardsIntoJobs();
        var emptySlots = trailingBlankJobs().length;
        if (opts && opts.replaceFromRaw) {
            var el = experienceEl();
            var parsed = parseJobs(el ? el.value : '');
            var filled = parsed.filter(function (job) { return !isBlankJob(job); }).map(normalizeJob);
            var attrBlanks = el ? Number(el.getAttribute('data-qc-blank-jobs') || 0) : 0;
            if (!Number.isFinite(attrBlanks) || attrBlanks < 0) attrBlanks = 0;
            if (attrBlanks > emptySlots) emptySlots = attrBlanks;
            // Sample/draft loads replace content; keep any empty cards the user just added
            // unless the caller opts out (explicit wipe).
            if (opts.wipeEmpty) {
                jobs = filled.length ? filled : [emptyJob('job-seed-0')];
                if (el) el.setAttribute('data-qc-blank-jobs', '0');
            } else {
                jobs = filled.slice();
                while (emptySlots > 0) {
                    jobs = jobs.concat([emptyJob(nextJobId())]);
                    emptySlots -= 1;
                }
                if (!jobs.length) jobs = [emptyJob('job-seed-0')];
            }
        }
        ensureJobs();
    ...
    function jobCardEl(index) {
        ...
        var targetJob = jobs[index];
        if (targetJob && targetJob.id) {
            for (var i = 0; i < cards.length; i++) {
                if (cards[i].getAttribute('data-job-id') === String(targetJob.id)) {
                    return cards[i];
                }
            }
        }
    ...
    function focusJobCard(index) {
        ...
        window.setTimeout(function () {
            try { card.classList.remove('is-just-added'); } catch (err) { /* ignore */ }
        }, 1600);
        ...
        // Nested studio scrollport — scrollIntoView alone often misses #studio-form-column.
        if (formCol) {
            try {
                var colRect = formCol.getBoundingClientRect();
                var cardRect = card.getBoundingClientRect();
                var nextTop = formCol.scrollTop + (cardRect.top - colRect.top) - Math.max(24, colRect.height * 0.12);
                formCol.scrollTo({ top: Math.max(0, nextTop), behavior: 'smooth' });
            } catch (scrollErr) {
                try { card.scrollIntoView({ behavior: 'smooth', block: 'center' }); } catch (err2) { /* ignore */ }
            }
        } else if (typeof card.scrollIntoView === 'function') {
            try { card.scrollIntoView({ behavior: 'smooth', block: 'center' }); } catch (err) { /* ignore */ }
        }
        if (focusEl && typeof focusEl.focus === 'function') {
            window.setTimeout(function () {
                try { focusEl.focus({ preventScroll: true }); } catch (err) {
                    try { focusEl.focus(); } catch (err2) { /* ignore */ }
                }
            }, 120);
        }
    }
    ...
    function resolveJobIndex(idOrIndex) {
        ...
        if (key === '1' || key.indexOf('job-seed') === 0 || key === 'job-slot-0') return 0;
        var idx = Number(idOrIndex);
        if (Number.isFinite(idx) && idx >= 0 && idx < jobs.length && String(idx) === key) return idx;
        return -1;
    }

    function addExperience(e) {
        // Accept DOM events, React flags, or bare calls from onclick/keyboard.
        var evt = (e && typeof e === 'object') ? e : null;
        if (evt && evt.__qcFromReact) {
            collectCardsIntoJobs();
            var fromCtx = (global.QCCvData && Array.isArray(QCCvData.experience)) ? QCCvData.experience : null;
            if (fromCtx && fromCtx.length > jobs.length) {
                jobs = fromCtx.map(function (row) {
                    return normalizeJob({
                        ...
                        position: row.role || row.title || row.position || '',
                ...
                announceJobAdded(jobs.length);
            }
            return false;
        }
        // Same DOM event only — never time-debounce (touch+click was swallowing taps).
        if (evt && evt.__qcForceAdd) {
            // Forced path from addJobBoot — skip event-handled guards.
        } else {
            if (evt && evt.__qcJobHandled) return false;
            if (addInFlight) return false;
            if (evt) {
                evt.__qcJobHandled = true;
                if (typeof evt.preventDefault === 'function') evt.preventDefault();
            }
        }
        if (addInFlight && !(evt && evt.__qcForceAdd)) return false;
        addInFlight = true;
        try {
            try { collectCardsIntoJobs(); } catch (collectErr) { /* keep in-memory jobs */ }
            ensureJobs();
            if (jobs.length >= MAX_JOBS) return false;
            var newJob = emptyJob(Date.now().toString() + '-' + Math.random().toString(36).slice(2, 7));
            jobs = (jobs || []).concat([newJob]);
            visibleJobsCount = jobs.length;
            ensureJobs();
            renderJobs();
            // Keep every card open so a new empty job is obvious (esp. next to another blank).
            var host = document.getElementById('experience-items');
            if (host) {
                Array.prototype.forEach.call(host.querySelectorAll('.experience-card'), function (card) {
                    card.classList.add('is-open');
                    var toggleBtn = card.querySelector('[data-toggle-job]');
            ...
            try { console.error('addExperience failed', addErr); } catch (logErr) { /* ignore */ }
        } finally {
            addInFlight = false;
        }
        return false;
    ...
    function announceJobAdded(count) {
        var status = document.getElementById('experience-add-status');
        if (!status) {
            status = document.createElement('p');
            status.id = 'experience-add-status';
            status.className = 'experience-add-status';
            status.setAttribute('role', 'status');
            status.setAttribute('aria-live', 'polite');
            var form = document.getElementById('experience-add-form');
            var btn = document.getElementById('btn-add-experience');
            if (form && form.parentNode) form.parentNode.insertBefore(status, form);
            else if (btn && btn.parentNode) btn.parentNode.insertBefore(status, btn);
        }
        var pack = t();
        status.textContent = (pack.jobN || 'משרה') + ' ' + count + ' ✓';
        status.classList.add('is-visible');
        window.clearTimeout(announceJobAdded._timer);
        announceJobAdded._timer = window.setTimeout(function () {
    ...
    function removeJob(idOrIndex) {
        collectCardsIntoJobs();
        var idToDelete = String(idOrIndex == null ? '' : idOrIndex);
        var next = (jobs || []).filter(function (item) {
            return item && String(item.id) !== idToDelete;
        });
        // Fallback: if no id matched, treat the arg as an index.
        if (next.length === jobs.length) {
            var index = resolveJobIndex(idOrIndex);
            if (index >= 0 && index < jobs.length) {
                next = jobs.filter(function (_item, i) { return i !== index; });
            }
        }
        jobs = next.length ? next : [emptyJob(Date.now().toString())];
        visibleJobsCount = jobs.length;
        ensureJobs();
        renderJobs();
        writeExperience(jobs, true);
    ...
    function syncPresetChipState(card, description) {
        if (!card) return;
        var text = String(description || '');
        var list = presets();
        Array.prototype.forEach.call(card.querySelectorAll('.experience-preset-chip'), function (chip) {
            var id = chip.getAttribute('data-preset-id');
            var preset = null;
            var i;
            for (i = 0; i < list.length; i++) {
                if (list[i].id === id) preset = list[i];
            }
            var snippet = preset ? descriptionFromBullets(preset.bullets) : '';
            var on = !!(snippet && text.indexOf(snippet) !== -1);
    ...
    function applyPreset(idOrIndex, presetId) {
        ...
        var current = String(job.description || '');
        var pos = incoming ? current.indexOf(incoming) : -1;
        var alreadyOn = pos !== -1;

        if (alreadyOn) {
            // Toggle OFF: remove this exact bullet snippet from the description.
            current = current.slice(0, pos) + current.slice(pos + incoming.length);
            current = current.replace(/\n{3,}/g, '\n\n').replace(/^\n+/, '').replace(/\n+$/, '');
            job.description = current;
            job.bullets = bulletsFromText(job.description);
            writeExperience(jobs, false);
            renderJobs();
            syncPresetChipState(jobCardEl(index), job.description);
        ...
        // Toggle ON: optionally fill blank role/company/dates, append bullets, mark active.
        ...
        current = String(job.description || '').replace(/\s+$/, '');
        ...
        syncPresetChipState(card, job.description);
        if (typeof global.updateCV === 'function') {
            try { global.updateCV(); } catch (err) { /* keep cards */ }
        }
    }
    ...
    function patchCardI18n() {
        ...
        Array.prototype.forEach.call(cards, function (card, jobIndex) {
            ...
            if (remove) {
                ...
                if (remove.classList.contains('exp-block-remove')) {
                    remove.innerHTML = trashSvg() + '<span>' + pack.removeJob + '</span>';
                }
            }
    ...
    function applyI18n() {
        ...
        var hint = document.getElementById('experience-section-hint');
        if (hint) {
            hint.textContent = global.QCCvLang === 'en'
                ? 'Each job is its own block — tap below to add another'
                : 'כל משרה בבלוק נפרד — לחצו למטה להוספה';
        }
        applyRawUi();
        if (rawMode) return;
        var host = document.getElementById('experience-items');
    ...
    function bindExperienceHost() {
        ...
        host.addEventListener('click', function (e) {
            ...
            if (btn.hasAttribute('data-toggle-job') || btn.closest('[data-toggle-job]')) {
    ...
    function ensureAddButtonVisible() {
        ...
        if (items && form && form.previousElementSibling !== items && form.previousElementSibling !== document.getElementById('experience-add-status')) {
            var status = document.getElementById('experience-add-status');
            if (status) items.insertAdjacentElement('afterend', status);
            var anchor = document.getElementById('experience-add-status') || items;
    ...
    function wireAddButtonEl(addJobBtn) {
        if (!addJobBtn) return;
        addJobBtn.type = 'button';
        addJobBtn.setAttribute('type', 'button');
        addJobBtn.setAttribute('data-qc-add-job', '1');
        addJobBtn.disabled = false;
        addJobBtn.hidden = false;
        addJobBtn.removeAttribute('disabled');
        addJobBtn.removeAttribute('hidden');
        addJobBtn.removeAttribute('aria-disabled');
        addJobBtn.removeAttribute('data-qc-add-bound');
        try { delete addJobBtn.dataset.qcAddBound; } catch (err) { /* ignore */ }
        addJobBtn.setAttribute('aria-controls', 'experience-items');
        addJobBtn.classList.add('exp-add-bar');
        addJobBtn.style.setProperty('pointer-events', 'auto', 'important');
        addJobBtn.style.setProperty('cursor', 'pointer', 'important');
        addJobBtn.style.setProperty('z-index', '200', 'important');
    }

    function bindAddButton() {
        var addJobBtn = document.getElementById('btn-add-experience');
        wireAddButtonEl(addJobBtn);
        var top = document.getElementById('btn-add-experience-top');
        if (top) {
            top.hidden = true;
            top.style.display = 'none';
        }
        // Direct click — never depend on form submit (other capture handlers
        // were canceling submit after we bailed out for type=submit).
        if (addJobBtn && !addJobBtn.__qcExpWired) {
            addJobBtn.__qcExpWired = true;
            addJobBtn.addEventListener('click', function (e) {
                if (global.__qcClickAddJob) global.__qcClickAddJob(e || window.event);
                else addExperience(e || window.event);
            });
        }
    }

    function bindDocumentAdd() {
        if (global.__qcAddJobDocBound === 'exp-ui2') return;
        global.__qcAddJobDocBound = 'exp-ui2';
        global.__qcAddJobDelegated = true;
        document.addEventListener('click', function (e) {
            var t = e && e.target;
            if (!t) return;
            var el = t.nodeType === 1 ? t : t.parentElement;
            if (!el || typeof el.closest !== 'function') return;
            var btn = el.closest('#btn-add-experience, [data-qc-add-job]');
            if (!btn) return;
            if (global.__qcClickAddJob) global.__qcClickAddJob(e);
            else addExperience(e);
        }, true);
    }
    ...
    function bind() {
        ...
        var filled = (jobs || []).filter(function (job) { return !isBlankJob(job); });
        jobs = filled.length ? filled.map(normalizeJob) : [emptyJob('job-seed-0')];
        visibleJobsCount = jobs.length;
        ensureJobs();
        // Always rebuild cards from state so the add button and toolbars stay wired.
        // Keeping the static seed HTML left the editor half-initialized for empty forms.
        if (!filled.length) {
            collectCardsIntoJobs();
            ensureJobs();
            if (!jobs.length) jobs = [emptyJob('job-seed-0')];
            if (jobs[0] && !jobs[0].id) jobs[0].id = 'job-seed-0';
        }
        renderJobs();
        applyI18n();
        bindRawToggle();
        ...
        names.forEach(function (name) {
            wrap(name, function () {
                rawMode = false;
                refreshJobsFromTextarea({
                    replaceFromRaw: name === 'fillCvSample' || name === 'loadExample' || name === 'loadRoleExample' || name === 'replaceRoleExample' || name === 'clearCvForm' || name === 'resetForm',
                    wipeEmpty: name === 'fillCvSample' || name === 'loadExample' || name === 'loadRoleExample' || name === 'replaceRoleExample' || name === 'clearCvForm' || name === 'resetForm'
                });
                applyI18n();
            });
        });
    ...
    var api = {
        init: bind,
        render: renderJobs,
        refreshJobsFromTextarea: refreshJobsFromTextarea,
        sync: collectCardsIntoJobs,
        refresh: function (opts) {
            // Default: sync from visible cards. Never wipe empty slots unless
            // the caller explicitly asks for replaceFromRaw (sample/draft load).
            return refreshJobsFromTextarea(opts || {});
        },
        applyI18n: applyI18n,
        ...
        previewJobs: previewJobs,
            ...
            // Trust the visible cards first so newly added empty jobs are kept.
            collectCardsIntoJobs();
            ensureJobs();
            var host = document.getElementById('experience-items');
            var cards = host ? host.querySelectorAll('.experience-card').length : 0;
            if (!host || cards !== jobs.length) {
                renderJobs();
            } else {
                Array.prototype.forEach.call(host.querySelectorAll('.experience-card'), function (card) {
                    card.classList.add('is-open');
    ...
    // Stable entry point for inline onclick — never strip this from the buttons.
    global.__qcClickAddJob = function __qcClickAddJob(e) {
        try {
            return addExperience(e);
        } catch (err) {
            try { console.error('__qcClickAddJob failed', err); } catch (logErr) { /* ignore */ }
            return false;
        }
    };

    global.QCCvData = global.QCCvData || {};
    if (!Array.isArray(global.QCCvData.experience) || global.QCCvData.experience.length < 1) {
        global.QCCvData.experience = [{ id: 'job-seed-0', title: '', company: '', dates: '', description: '' }];
    }
    jobs = [emptyJob('job-seed-0')];
    visibleJobsCount = 1;
    ensureJobs();
    publishExperience();

    /** Imperative FormData-style setter for the experience array (mirrors React setFormData). */
    global.setCvFormData = function setCvFormData(updater) {
        global.QCCvData = global.QCCvData || {};
        var prev = global.QCCvData;
        var next = typeof updater === 'function' ? updater(prev) : Object.assign({}, prev, updater || {});
        global.QCCvData = next;
        if (Array.isArray(next.experience)) {
            jobs = next.experience.map(function (row) {
                return normalizeJob({
                    id: row.id || Date.now().toString(),
                    title: row.role || row.title || '',
                    company: row.company || '',
            ...
            if (!jobs.length) jobs = [emptyJob(Date.now().toString())];
            visibleJobsCount = jobs.length;
            ensureJobs();
            renderJobs();
            writeExperience(jobs, true);
