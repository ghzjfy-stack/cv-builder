(function (global) {
    ...
    function toTxt(data) {
        ...
        if (global.QCCoverLetter && QCCoverLetter.enabled && QCCoverLetter.enabled()) {
            lines.push('', isEnglish() ? 'Cover Letter' : 'מכתב מקדים', QCCoverLetter.buildText());
        }
    ...
    function buildDocx(data) {
        ...
        var body = paraXml(data.name, { size: 44, bold: true, after: 40 }) +
            ...
            (global.QCCoverLetter && QCCoverLetter.enabled && QCCoverLetter.enabled()
                ? headingXml(isEnglish() ? 'Cover Letter' : 'מכתב מקדים') + paraXml(QCCoverLetter.buildText())
                : '') +
    ...
    function exportImage(type) {
        ...
        var run = (typeof html2canvas === 'function')
            ? html2canvas(el, { scale: 2, useCORS: true, logging: false, scrollX: 0, scrollY: 0, backgroundColor: '#ffffff' })
