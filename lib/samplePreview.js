(function (global) {
    ...
    function cvThemeSource() {
        if (typeof global.QCCvThemeRoot === 'function') {
            var root = global.QCCvThemeRoot();
            if (root) return root;
        }
    ...
    function copyVars(node) {
        var accentSrc = cvThemeSource();
        var fontSrc = document.documentElement;
        ['--accent', '--cv-accent-color'].forEach(function (name) {
            var value = accentSrc.style.getPropertyValue(name) || getComputedStyle(accentSrc).getPropertyValue(name);
            if (value) node.style.setProperty(name, value.trim());
        });
        ['--cv-font', '--cv-scale', '--cv-leading', '--cv-density'].forEach(function (name) {
            var value = fontSrc.style.getPropertyValue(name) || getComputedStyle(fontSrc).getPropertyValue(name);
            if (value) node.style.setProperty(name, value.trim());
        });
        var accent = (node.style.getPropertyValue('--accent') || node.style.getPropertyValue('--cv-accent-color') || '').trim();
        if (accent) {
            node.style.setProperty('--accent', accent);
            node.style.setProperty('--cv-accent-color', accent);
        }
    }
    ...
    function downloadFromPreview(e) {
        ...
        closeModal();
        window.setTimeout(function () {
            var paid = document.documentElement.classList.contains('qc-paid') || !!global.QCIsPaid;
            if (paid && typeof global.triggerPDFDownload === 'function') {
                global.triggerPDFDownload();
                return;
            }
            if (paid && typeof global.onDownloadPdfClick === 'function') {
                global.onDownloadPdfClick();
                return;
            }
