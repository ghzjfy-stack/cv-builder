(function (global) {
    ...
    var BADGES = {
        ...
        terracotta: { kind: "new", he: "חדש", en: "New" },
        ...
        "creative-split": { kind: "new", he: "חדש", en: "New" },
    ...
    function badgeOf(tpl) {
        ...
        if (tags.indexOf("minimal") >= 0) return { kind: "minimal", he: "נקי ומינימליסטי", en: "Minimal" };
    ...
    function badgeHtml(tpl, lang, badge) {
        badge = badge || badgeOf(tpl);
    ...
    function cardHtml(tpl, lang, id) {
        ...
        var badgeLabel = lang === "en" ? badge.en : badge.he;
        var titleText = String(title).trim();
        if (badge.he === "יצירתי" && (titleText === badgeLabel || /יצירתי/.test(titleText))) {
            badge = { kind: "new", he: "חדש", en: "New" };
            badgeLabel = lang === "en" ? badge.en : badge.he;
        }
        var badgeMark = titleText === String(badgeLabel).trim() ? "" : badgeHtml(tpl, lang, badge);
        return '<article class="template-card" data-example="' + id + '" data-ats="' + (tpl.atsOptimized ? "1" : "0") + '" aria-pressed="false">' +
            badgeMark + thumbHtml(tpl, lang) +
