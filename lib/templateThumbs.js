(function (global) {
    ...
    function paperOf(tpl) {
        var bg = tpl.bg || "";
        if (bg.indexOf("cream") >= 0) return "#f7f3ec";
        if (bg.indexOf("linen") >= 0) return "#f4eee4";
        if (bg.indexOf("mint") >= 0) return "#f4faf8";
        if (bg.indexOf("pearl") >= 0) return "#f5f2ed";
        if (bg.indexOf("slate") >= 0) return "#eef1f4";
        return hex(tpl.colorPalette && tpl.colorPalette.background, "#ffffff");
    }
    function inkOf(tpl) {
