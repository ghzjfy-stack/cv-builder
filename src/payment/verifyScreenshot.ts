...
async function compressIfNeeded(file: File): Promise<{ imageBase64: string; mimeType: string }> {
...
export async function verifyPaymentScreenshot(file: File): Promise<VerifyPaymentResponse> {
  ...
  try {
    data = (await res.json()) as VerifyPaymentResponse;
  } catch {
    data = null;
  }
